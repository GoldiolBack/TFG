...
for i in f_test_l1c:
	...
	# EXPORT RASTER TIFF
	raster = rasterio.open("/mnt/gpid07/users/oriol.esquena/images_sentinel/test/"+"20m/"+date+".tiff", "w", driver="Gtiff",
...
for i in f_train_l1c:
	...
	# EXPORT RASTER TIFF
	raster = rasterio.open("/mnt/gpid07/users/oriol.esquena/images_sentinel/train/"+"20m/"+date+".tiff", "w", driver="Gtiff",
	                       width=band5.width, height=band5.height,
	                       count=6, crs=band5.crs, transform=band5.transform,
	                       dtype=band5.dtypes[0])
	# raster.write(band4.read(1), 1)  # write band4 -red- in position 1
